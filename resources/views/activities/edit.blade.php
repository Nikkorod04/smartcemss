<x-admin-layout header="Edit Activity">
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-md p-8">
                <h1 class="text-3xl font-bold text-gray-900 mb-8">Edit Activity</h1>

                <form action="{{ route('activities.update', $activity) }}" method="POST" class="space-y-8">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="extension_program_id" name="extension_program_id" required
                                ...
                                @foreach($programs as $program)
                                    <option value="{{ $program->id }}" @if(old('extension_program_id', $activity->extension_program_id) == $program->id) selected @endif>
                                        {{ $program->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <x-text-input id="title" class="block w-full mt-1" type="text" name="title" 
                                             placeholder="e.g., Week 1 Tutoring Sessions" value="{{ old('title', $activity->title) }}" required />
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-text-input id="planned_start_date" class="block w-full mt-1" type="date" 
                                                    name="planned_start_date" value="{{ old('planned_start_date', $activity->planned_start_date?->format('Y-m-d')) }}" />
                                    ...
                                    <div>
                                        ...
                                        <x-text-input id="planned_end_date" class="block w-full mt-1" type="date" 
                                                    name="planned_end_date" value="{{ old('planned_end_date', $activity->planned_end_date?->format('Y-m-d')) }}" />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="actual_start_date" class="block w-full mt-1" type="date" 
                                                name="actual_start_date" value="{{ old('actual_start_date', $activity->actual_start_date?->format('Y-m-d')) }}" required />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="actual_end_date" class="block w-full mt-1" type="date" 
                                                name="actual_end_date" value="{{ old('actual_end_date', $activity->actual_end_date?->format('Y-m-d')) }}" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="venue" class="block w-full mt-1" type="text" name="venue" 
                                             placeholder="e.g., San Juan Community Center" value="{{ old('venue', $activity->venue) }}" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="allocated_budget" class="block w-full mt-1" type="number" name="allocated_budget" 
                                             placeholder="0.00" step="0.01" value="{{ old('allocated_budget', $activity->allocated_budget) }}" required />
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select id="status" name="status" required
                                        class="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    <option value="pending" @if(old('status', $activity->status) == 'pending') selected @endif>Pending</option>
                                    <option value="ongoing" @if(old('status', $activity->status) == 'ongoing') selected @endif>Ongoing</option>
                                    <option value="completed" @if(old('status', $activity->status) == 'completed') selected @endif>Completed</option>
                                </select>
                            ...
                            @forelse($faculties as $faculty)
                                <label class="flex items-center p-3 hover:bg-gray-50 rounded-lg cursor-pointer transition">
                                    <input type="checkbox" name="faculties[]" value="{{ $faculty->id }}" 
                                          @if(in_array($faculty->id, $selectedFaculties)) checked @endif
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3"
                                 ...
                                 class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $activity->notes) }}</textarea>
                    ...
                    <div class="flex gap-4 pt-6 border-t">
                        ...
                        <a href="{{ route('activities.show', $activity) }}" class="inline-flex items-center justify-center px-8 py-3 bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium rounded-lg transition">
                            Cancel
                        </a>
                    </div>
                </form>
